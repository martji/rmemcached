...
public class MemcachedClient {
	 ...
	 * @param serversMap
	 */
	public MemcachedClient(int clientId, int replicasNum, Map<Integer, ServerNode> serversMap) {
		this.clientId = clientId;
		this.replicasNum = replicasNum;
		init(serversMap);
	}
	
	public MemcachedClient(int clientId, int replicasNum) {
		this.clientId = clientId;
		this.replicasNum = replicasNum;
	}
	...
	public MClient randMClient(Integer hash) {
		if (replicasNum == 1) {
			return leaderClient(hash);
		}
		Random random = new Random();
		int clientsNum = clientMap.size();
		int leaderIndex = hash % clientsNum + clientsNum;
		int index = random.nextInt(replicasNum);
		for (int i = 0; i < replicasNum; i++) {
			int num = (leaderIndex + i + index) % clientsNum;
			if (num < 0) {
				num += clientsNum;
			}
	...
	public MClient leaderClient(Integer hash) {
		...
		int leaderIndex = hash % clientsNum + clientsNum;
		for (int i = 0; i < replicasNum; i++) {
			int num = (leaderIndex + i)	% clientsNum;
			if (num < 0) {
				num += clientsNum;
			}
			MClient mClient = clientMap.get(num);
